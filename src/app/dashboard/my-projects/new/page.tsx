...
export default function NewProjectPage() {
  ...
  const onSubmit = (data: ProjectFormValues) => {
    // حفظ البيانات مؤقتاً للانتقال لصفحة الدفع
    sessionStorage.setItem('pendingProject', JSON.stringify(data));
    router.push('/dashboard/my-projects/new/checkout');
  };
        ...
        <div className="order-1 flex flex-col gap-8 lg:order-1 lg:col-span-8">
          ...
          <Card className="border-muted/20 bg-card/50 backdrop-blur-sm">
                ...
                {errors.milestones && (
                  <span className="text-destructive text-xs font-medium">
                    {errors.milestones.message ||
                      (errors.milestones as any).root?.message}
                  </span>
                )}
              </div>
              <Button
            ...
            <CardContent className="space-y-6 pt-8">
              {fields.length === 0 && (
                <div className="text-destructive rounded-xl border-2 border-dashed py-8 px-4 text-center bg-destructive/5 font-bold space-y-2">
                  <p>يجب إضافة مراحل لتقسيم الدفعات (إلزامي).</p>
                  <p className="text-xs opacity-80">في حالة عدم الرغبة في تقسيم المشروع، يمكنك إضافة مرحلة واحدة باسم "التسليم" بكامل المبلغ.</p>
                </div>
              )}
